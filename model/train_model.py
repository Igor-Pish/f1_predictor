...
def train_logreg_model(years, random_state=0):
    # --- Сбор данных
    ...
    # --- Сохранение
    joblib.dump(model, "model/f1_win_logreg.pkl")
    print("✅ Модель LogReg обучена и сохранена")
...
def train_rf_model(years, random_state=0):
    # --- Сбор данных
    df = collect_all_data(years)
    X = df[["q1_norm", "q2_norm", "q3_norm", "quali_position", "driver", "team"]]
    y = df["target"]
    ...
    model = Pipeline([
        ("prep", preprocessor),
        ("clf", RandomForestClassifier(random_state=random_state, n_estimators=500, class_weight="balanced"))
    ])
    ...
    # --- Сохранение
    joblib.dump(model, "model/f1_win_rf.pkl")
    print("✅ Модель RandomForest обучена и сохранена")
...
def train_stacked_predictor(years, random_state=0, model_path="model/stacked_rf_logreg.pkl"):
    # --- Загружаем и делим
    df = collect_all_data(years)
    X = df[["q1_norm", "q2_norm", "q3_norm", "quali_position", "driver", "team"]]
    y = df["target"]
    ...
    rf_base = RandomForestClassifier(random_state=random_state, n_estimators=500, class_weight='balanced')
    rf_models = []
    ...
    print("✅ Модель Stacked обучена и сохранена")
